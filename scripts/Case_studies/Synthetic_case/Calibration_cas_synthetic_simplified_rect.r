...
Experiment_id <- c(
    ...
    # "Syn_case_rec_5_3_with_Ks_obs_limites_all_obs"
    # "Syn_case_without_ks_measures_all_obs"
    # 'Syn_case_without_ks_measures' # Syn_case_rec_5_3_with_Ks_obs_limites # Syn_case_rec_5_3_with_Ks_obs_limites_all_obs #Syn_case_without_ks_measures_all_obs
)
...
command_line_MAGE <- ""
file_main_path <- "/home/famendezrios/Documents/These/VSCODE-R/HydroBayes/HydroBayes_git/Case_studies/Synthetic_case/Simplified_ks_rectangular_MC/Calibration_experiments"
MAGE_main_folder <- "/home/famendezrios/Documents/These/VSCODE-R/HydroBayes/HydroBayes_git/Case_studies/Synthetic_case/Simplified_ks_rectangular_MC/model_mage"
...
load("/home/famendezrios/Documents/These/VSCODE-R/HydroBayes/HydroBayes_git/data/processed_data/Synthetic_case/WSE_synthetic_simplified_rectangle.RData")
...
WSE_synthetic_simplified_raw_event_1 <- WSE_synthetic_simplified %>%
    filter(id_case == "5_1", id_reach_CAL %in% Input_XR[[1]]) %>%
    mutate(
        id_reach = case_when(
...
for (i in 1:length(Input_XR)) {
    ...
    if (nrow(WSE_synthetic_simplified_Cal_event_1 %>%
        filter(id_reach_CAL %in% Input_XR[[i]])) > 0) {
        if (i == 1) {
            CalData_event_1 <- WSE_synthetic_simplified_Cal_event_1 %>%
            ...
            CalData_event_1_temp <- WSE_synthetic_simplified_Cal_event_1 %>%
                mutate(Reach_groupped_Cal = case_when(
                    id_reach_CAL %in% Input_XR[[i]] ~ names(Input_XR)[i],
                    TRUE ~ Reach_groupped_Cal
                )) %>%
                ...
                filter(!KP %in% nodes_to_remove_cal) %>%
                ungroup()
            CalData_event_1 <- rbind(CalData_event_1, CalData_event_1_temp)
        }
    }
}
...
WSE_Synthetic_Val_event_1 <- WSE_synthetic_simplified_raw_event_1 %>%
    anti_join(CalData_event_1, by = colnames(WSE_synthetic_simplified_raw_event_1)) %>%
    mutate(set = "validation")

# 3. Combine into a single data frame
WSE_Event_1 <- bind_rows(CalData_event_1, WSE_Synthetic_Val_event_1) %>% arrange(KP)
...
ggplot(CalData_event_1, aes(x = KP, y = WSE, col = factor(id_reach_CAL))) +
    geom_point() +
    geom_errorbar(aes(
        ymin = WSE - qnorm(0.975) * Yu,
        ymax = WSE + qnorm(0.975) * Yu
...
WSE_synthetic_simplified_raw_event_2 <- WSE_synthetic_simplified %>%
    filter(id_case == "3_2", id_reach_CAL %in% Input_XR[[2]]) %>%
    mutate(
        id_reach = case_when(
            id_reach_CAL == 1 ~ "Main reach upstream",
            id_reach_CAL == 2 ~ "Tributary",
...
for (i in 1:length(Input_XR)) {
    ...
    if (nrow(WSE_synthetic_simplified_Cal_event_2 %>%
        filter(id_reach_CAL %in% Input_XR[[i]])) > 0) {
        if (i == 1) {
            CalData_event_2 <- WSE_synthetic_simplified_Cal_event_2 %>%
                mutate(Reach_groupped_Cal = case_when(
                    id_reach_CAL %in% Input_XR[[i]] ~ names(Input_XR)[i],
                    TRUE ~ Reach_groupped_Cal
                )) %>%
            ...
            CalData_event_2_temp <- WSE_synthetic_simplified_Cal_event_2 %>%
                mutate(Reach_groupped_Cal = case_when(
                    id_reach_CAL %in% Input_XR[[i]] ~ names(Input_XR)[i],
                    TRUE ~ Reach_groupped_Cal
                )) %>%
                ...
                filter(!KP %in% nodes_to_remove_cal) %>%
                ungroup()
            CalData_event_2 <- rbind(CalData_event_2, CalData_event_2_temp)
        }
    }
}
...
WSE_Synthetic_Val_event_2 <- WSE_synthetic_simplified_raw_event_2 %>%
    anti_join(CalData_event_2, by = colnames(WSE_synthetic_simplified_raw_event_2)) %>%
    mutate(set = "validation")

# 3. Combine into a single data frame
WSE_Event_2 <- bind_rows(CalData_event_2, WSE_Synthetic_Val_event_2) %>% arrange(desc(KP))
...
Y$Discharge <- -9999
Y$Velocity <- -9999
if (Experiment_id == "Syn_case_rec_5_3_with_Ks_obs_limites" | Experiment_id == "Syn_case_rec_5_3_with_Ks_obs_limites_all_obs" | Experiment_id == "Syn_case_rec_5_3_with_Ks_obs_limites_all_obs_flat_prior") {
    Y$Y_Kmin <- ifelse(X$reach == 1 | X$reach == 3, 34, 28)
    ...
    Y$Y_Kmin[sliced_ks$idx] <- sliced_ks$k
...
if (Experiment_id == "Syn_case_rec_5_3_with_Ks_obs_limites" | Experiment_id == "Syn_case_rec_5_3_with_Ks_obs_limites_all_obs" | Experiment_id == "Syn_case_rec_5_3_with_Ks_obs_limites_all_obs_flat_prior") {
    ...
    toto <- cbind(X, Yu, Yu_Kmin = Yu_Kmin)
...
CalData_plot_export <- CalData_plot_export +
    facet_wrap(
        ...
        labeller = labeller(
            name_event = c(
                "3_MR_2_TR" = expression("Tributary: \n discharge of 1 m"^
                    {
                        3
                    } * "/s"),
                "5_MR_1_TR" = expression("Main reach: \n discharge of 5  m"^
                    {
                        3
                    } * "/s")
            )
...
if (Experiment_id == "Syn_case_rec_5_3_with_Ks_obs_limites" | Experiment_id == "Syn_case_rec_5_3_with_Ks_obs_limites_all_obs" | Experiment_id == "Syn_case_rec_5_3_with_Ks_obs_limites_all_obs_flat_prior" | Experiment_id == "Syn_case_with_manual_Ks_obs_limites_all_obs_flat_prior") {
...
save(
    file = file.path(path_experiment, "CalData_plot_WSE_observed.RData"),
    CalData_plot_export
)
