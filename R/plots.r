plot_CalData_lastest <- function(CalData,
                                 scales_free = "free_y",
                                 wrap = TRUE) {
    ##########################################
    ...
    if (any(CalData$WSE != -9999)) {
        ...
        if (wrap) {
            plot_WSE <- plot_WSE +
                facet_wrap(~ event + reach + target_datetime, scales = scales_free, ncol = 2)
        }
    ...
    if (any(CalData$Q != -9999)) {
        ...
        if (any(CalData$Yu_Q != 0)) {
            plot_Q <- plot_Q +
                geom_errorbar(aes(
                    ymin = Q - qnorm(0.975) * Yu_Q,
                    ymax = Q + qnorm(0.975) * Yu_Q
                ))
        }
        ...
        if (wrap) {
            plot_Q <- plot_Q +
                facet_wrap(~ event + reach + x, scales = scales_free, ncol = 2)
        }
