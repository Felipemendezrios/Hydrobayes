...
mage_extraire_args <- lapply(sequence_all, function(x) paste("YdT 1", format(x, nsmall = max_decimals)))
...
if (logical_WS_Calibration) {
    ...
    data_obs_calibration_merged_wide <- data.frame(data_obs_calibration %>%
        select(time_fixed, h_mean, id_position) %>%
        pivot_wider(names_from = id_position, values_from = h_mean))
...
param_theta <- list(
    ...
    # Kmin.prior.par = c(log(150), prior_log_u_ks_mage),
...
residuals_extraction <- residuals[, c(1, 3:11, 21:38)]
for (i in 1:ncol(residuals_extraction)) {
    residuals_extraction[which(residuals_extraction[, i] == -9999), i] <- NA
}

col_names <- paste0("Y", 1:9)
position_labels <- paste("Position:", sequence_all)
...
ggplot(res_long, aes(x = X1_obs, y = value, color = type)) +
    ...
    theme_bw() +
    labs(
        x = "Time (hours)", y = "WS profile (m)",
        title = "Comparison of simulation and observation at each position used during calibration "
    ) +
    theme(
...
ggplot(residuals_long, aes(x = X1_obs, y = value, color = type)) +
    ...
    theme_bw() +
    labs(
        x = "Time (hours)", y = "Residuals (m)",
        title = "Residual at each position used during calibration "
    ) +
    theme(
        strip.text = element_text(size = 12),
        axis.text.x = element_text(angle = 45, hjust = 1),
