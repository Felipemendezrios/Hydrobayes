...
# Estimation: constant in the main channel and constant in the floodplain but separated in two parts. 
...
- Assess the methodology to get a estimation of a piecewise function for the friction coefficient giving the separation manually.
...
```{r}
path <- "/home/famendezrios/Documents/These/VSCODE-R/HydroBayes/HydroBayes_git/Case_studies/HHLab/Compound_channel_spatial_friction/Calibration/piecewise_function_WSE_obs/"
...
WSE_data_temp2 <- WSE_data_temp[str_detect(WSE_data_temp$ID_experiment, pattern = case), ]

ggplot(WSE_data_temp2, aes(x = x, y = (h_mean - 0.115) * 1000, color = ID_experiment)) +
  geom_point() +
  labs(
    title = "Observations of water depth",
    x = "Streamwise distance (m)",
    y = "Water depth (mm)",
    color = "Experiment"
  ) +
  facet_wrap(~ID_channel, ncol = 2) +
  theme_bw() +
  theme(
...
Instabilities have been detected. No explanation has been found for this problem. 
...
ggplot() +
  ...
  labs(
    title = "Friction coefficient estimation in the main channel \nwith parametric uncertainty (Zoom)",
    x = "Lengthwise position (meters)",
    y = expression("Friction coefficient (m"^
      {
        1 / 3
...
The MAP estimation is lower than the literature values to the glass friction coefficient. 
...
The first part of the floodplain (wood) is estimated between the literature values. However, the second part of the floodplain (grass) is not coherent to the literature values, increasing instead of decreasing as expected.
...
# Estimation with the same uncertainty for WSE observations but introducing a pseudo-observation for the friction coefficient in the main channel

## Check MCMC 
```{r}
path <- "/home/famendezrios/Documents/These/VSCODE-R/HydroBayes/HydroBayes_git/Case_studies/HHLab/Compound_channel_spatial_friction_test/Calibration/piecewise_function_WSE_Kmin_obs/"
...
Pseudo-observations force the estimation of the frictio in the main channel closer to observations. However, instabilites have been detected for the floodplain friction coefficients.
...
As expected, observation on the friction coefficient in the main channel lead to a good estimation for this parameter.
...
In floodplain, instabilites during the MCMC sampling do not analyse the results. That's why a zoom have been apply to the literature values and results show that estimation of the friction coefficient upstream is sligth lower than the literature values. However, estimation is not coherent with the literature values for the second part of the floodplain (grass).
...
-  Estimation of the piecewise function for the friction coefficient in the floodplain and constant friction in the main channel is not working properly. 
-  The estimation of the friction coefficient in the floodplain is not coherent with the literature values.
-  High degree of freedom would lead to instabilites during the MCMC sampling.
-  As @dupuis_compound_2017 suggested, the water surface level is primarily controlled by the downstream weir levels, over the downstream roughness. 
