...
all_experiments <- c(
    "1_WSE_main_channel_real_uncertainty"
)
...
file_main_path <- "/home/famendezrios/Documents/These/VSCODE-R/HydroBayes/HydroBayes_git/Case_studies/HHLab/Rectangular_channel_1_event/Calibration_experiments"
...
plot_DIC <- function(
    ...
    DIC_plot <- ggplot(DIC_results, aes(x = factor(Degree), y = Value, col = factor(Criteria))) +
        ...
        annotate("segment",
            x = factor(min_local$Degree), y = min_local$Value * 1.02, xend = factor(min_local$Degree), yend = min_local$Value * 1.009,
