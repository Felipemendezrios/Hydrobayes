...
path_temp_results <- tempdir()
path_temp_results

# dir_temp_calibration_test <- "/home/famendezrios/Documents/These/VSCODE-R/HydroBayes/HydroBayes_git/Development/New_features_Ben/"
dir_temp_calibration_test <- "/home/famendezrios/Documents/These/VSCODE-R/HydroBayes/HydroBayes_git/Development/New_features_Ben_spatial/"

CalData <- read.table(file.path(dir_temp_calibration_test, "CalibrationData.txt"), header = TRUE)
X <- CalData[, 1:4]
...
# Reach 2 : main channel
a0_min_reach_2 <- RBaM::parameter(
    name = "a0_min_r_2",
    init = 1 / 0.010, # Initial guess
    prior.dist = "FlatPrior+", # Prior distribution
    prior.par = NULL
...
a1_flood_reach_2 <- RBaM::parameter(
    name = "a1_flood_r_2",
    init = 0.1,
    prior.dist = "Uniform",
    prior.par = c(-5, 5)
...
if (n_degree_Kmin == 0 && n_degree_Kflood == 0) {
    if (case == "independant") {
        Kmin_prior <- list(
            a0_min_reach_1, # reach 1
            a0_min_reach_2 # reach 2
            # a1_min
        )
        Kflood_prior <- list(
            a0_flood_reach_1, # reach 1
            a0_flood_reach_2 # reach 2
        )
    } else if (case == "follow") {
        Kmin_prior <- list(
            a0_min_reach_1 # reach 1
...
} else if (n_degree_Kmin == 1 && n_degree_Kflood == 0) {
    if (case == "independant") {
        Kmin_prior <- list(
            a0_min_reach_1, # reach 1
            a1_min_reach_1,
...
} else if (n_degree_Kmin == 1 && n_degree_Kflood == 1) {
    if (case == "independant") {
        Kmin_prior <- list(
            a0_min_reach_1, # reach 1
            a1_min_reach_1,
...
mageDir <- c(
    paste0(dir_temp_calibration_test, "model_mage_event_main/"),
    paste0(dir_temp_calibration_test, "model_mage_event_2/"),
    paste0(dir_temp_calibration_test, "model_mage_event_3/"),
    paste0(dir_temp_calibration_test, "model_mage_event_4/")
)
